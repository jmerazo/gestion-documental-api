...
class ImapReader
{
    ...
    public function getMessages($type = 'text')
    {
        ...
        $emails = imap_sort($stream, SORTDATE, 1, 0, 'ALL');
        $messages = array();
        if ($emails) {
            $this->emails = $emails;
            $cont = 0;
    ...
    private function loadMessage($uid, $type)
    {
        ...
        if ($type === 'text')
            $this->plaintextMessage = $this->get_part($this->imapStream, $uid, "TEXT/PLAIN", $structure);
        else {
            $this->htmlMessage = $this->get_part($this->imapStream, $uid, "TEXT/HTML", $structure);
        }
